...
export default function Navbar() {
  ...
  useEffect(() => {
    // Sin sesión no hay nada que contar, y consultar igual dispara un 401 cada 30 segundos
    // contra el backend mientras el usuario mira la pantalla de login.
    const eqId = equipoSeleccionado?.id;
    if (!isAuthenticated || !eqId) {
      setPendientes(0);
      return;
    }
    ...
    const cargar = async () => {
      try {
        ...
        // Campos explícitos y nada de `JSON.stringify(datosPropuestos).includes(eqId)`: ese
        // match por substring contaba como propia cualquier solicitud que mencionara el id del
        // equipo en cualquier campo (el rival de un partido, por ejemplo), así que el badge
        // mostraba pendientes que no eran de este equipo.
        const count = (resp.solicitudes || []).filter((s: any) => {
          const dp = s?.datosPropuestos || {};
          return s?.entidad === eqId || dp?.equipoId === eqId || dp?.equipo === eqId;
        }).length;
  ...
  }, [equipoSeleccionado?.id, isAuthenticated]);
      ...
      {open && (
        <div className="border-t border-slate-200 bg-white px-4 py-3 lg:hidden">
          <div className="flex flex-col gap-2">
            ...
            {/* La campanita de solicitudes sólo estaba en la barra de escritorio: en un celular,
                que es donde el DT usa esto de verdad, no había forma de ver las notificaciones
