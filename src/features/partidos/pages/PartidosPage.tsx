...
const PartidosPage = () => {
  ...
  const [crearForm, setCrearForm] = useState<CrearFormState>(crearFormDefault);
  ...
  const handleCrearChange = (event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
  ...
  const handleCrearPartido = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!crearForm.rivalId || !crearForm.fecha || !crearForm.modalidad || !crearForm.categoria) {
  ...
  const handleSeleccionarEstado = async (estado: Partido['estado']) => {
            ...
            <div className="space-y-4">
              {recientes.slice(0, 10).map((partido) => (
                <button
                  ...
                  className="w-full text-left"
                >
                  <PartidoCard partido={partido} variante="resultado" />
                </button>
              ))}
            </div>
          ) : (
